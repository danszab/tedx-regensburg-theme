...
<head>
	...
	<?php wp_head(); ?>
	<?php
	$ticket_url  = tedx_mod( 'tedx_ticket_url', '#' );
...
<header id="site-header" class="fixed top-0 left-0 right-0 z-50 w-full transition-all duration-300">
	<div class="max-w-figma-wide mx-auto flex items-center justify-between h-[60px] lg:h-[70px] px-4 md:px-8 lg:px-[64px]">
		
		<!-- Left Group: Logo -->
		<div class="flex-shrink-0 flex items-center">
			<?php tedx_regensburg_logo( 'h-7 lg:h-8 w-auto' ); ?>
		</div>

		<!-- Center Group: Desktop Navigation Menu -->
		<nav class="hidden lg:flex flex-1 items-center justify-center min-w-0 px-4" aria-label="<?php esc_attr_e( 'Primary Menu', 'tedx-regensburg' ); ?>">
			<?php
			if ( has_nav_menu( 'primary' ) ) {
				wp_nav_menu( array(
					'theme_location' => 'primary',
					'container'      => false,
					'menu_class'     => 'flex items-center justify-center gap-6 list-none m-0 p-0',
					'walker'         => new TEDx_Tailwind_Nav_Walker(),
					'fallback_cb'    => false,
				) );
			} else {
				?>
				<ul class="flex items-center justify-center gap-6 list-none m-0 p-0">
